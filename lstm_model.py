...
class LSTMAutoencoder(nn.Module):
    def __init__(self, input_dim, hidden_dim, num_layers, dropout=0.0, bidirectional=False):
        ...
        lstm_hidden = hidden_dim * (2 if bidirectional else 1)
        ...
        self.output_layer = nn.Linear(lstm_hidden, input_dim)
    ...
    def forward(self, x):
        ...
        if self.bidirectional:
            ...
            last = torch.cat([forward_last, backward_last], dim=-1)
...
def train_and_predict(train_data, test_data):
    ...
    bidirectional = CONFIG["lstm"].get("bidirectional", False)
